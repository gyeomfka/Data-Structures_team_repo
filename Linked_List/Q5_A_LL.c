...
#include <stdlib.h>
/**
 * 단일 연결 리스트를 두 개의 부분 리스트로 분할하는 C 함수 frontBackSplitLL()을 작성하세요.
 * 요구사항
	분할 방식:
	앞쪽 절반(front half)을 위한 리스트 하나
	뒤쪽 절반(back half)을 위한 리스트 하나

	홀수 개수 처리:
	요소의 개수가 홀수인 경우, 추가 요소는 **앞쪽 리스트(frontList)**에 포함되어야 합니다

	출력:
	frontBackSplitLL() 함수는 frontList와 backList를 포함하는 두 리스트를 출력합니다

	함수 프로토타입
	void frontBackSplitLL(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList);

	예시
	입력 연결 리스트: 2, 3, 5, 6, 7 (총 5개 - 홀수)
	결과:
		frontList: 2, 3, 5 (3개 - 홀수이므로 앞쪽에 1개 더)
		backList: 6, 7 (2개)

	추가 정보
	fixsize = ll->size (원본 리스트의 크기 사용)
 */
//////////////////////////////////////////////////////////////////////////////////
...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	int splitCountFront = ll->size / 2;
	if (ll->size % 2 != 0) {
		splitCountFront++;
	}
	int splitCountBack = ll->size - splitCountFront;

	ListNode *cur = ll->head;
	ListNode *frontHead = resultFrontList->head, *backHead = resultBackList->head;

	int nowCnt = 0;

	ListNode *frontTemp = NULL, *backTemp = NULL;
	...
	while (cur != NULL) {
		...
		if (nowCnt == 0) {
			...
			frontTemp = resultFrontList->head;
			resultFrontList->size++;
		} else if (nowCnt < splitCountFront) {
			resultFrontList->head->next = newNode;
			resultFrontList->head = resultFrontList->head->next;
			resultFrontList->size++;
		} else if (nowCnt == splitCountFront) {
			resultBackList->head = newNode;
			backTemp = resultBackList->head;
			resultBackList->size++;
		} else {
			resultBackList->head->next = newNode;
	...
	resultFrontList->head = frontTemp;
	resultBackList->head = backTemp;
}
