...
{
    /*
		연결리스트를 순회하며 최댓값 기억한 후 가장 앞으로 이동
		while(next가 not null){
			max값과 비교 -> max값이다 -> index 저장
		}
		max노드의 index값으로 find
		delete + 제일앞에 insert
	*/
	ListNode *cur;
	if(ptrHead == NULL){
	...
	LinkedList *ll = (LinkedList*)(ptrHead - 1);
	...
	if(maxIndex > 0){
		// ptrHead의 값 바꾸고 기존의 head주소를 next로
		ListNode *maxNode = findNode(ll, maxIndex);
		insertNode(ll, 0, maxNode->item);
		removeNode(ll, maxIndex + 1);
	}
